...
const navigationItems = [
  ...
  { icon: Flame, href: "/health-streak", label: "Health Streak", category: "health" },
  { icon: BookOpen, href: "/education", label: "Education Hub", category: "health" },
  { icon: Video, href: "/consultations", label: "E-Consultations", category: "health" },
  { icon: Pill, href: "/medicine", label: "Medicine Finder", category: "health" },
  { icon: MessageCircle, href: "/chat", label: "AI Health Assistant", category: "health" },
  { icon: Grid3X3, href: "/patients", label: "Patients", category: "management" },
  { icon: BarChart3, href: "/records", label: "Records", category: "management" },
  { icon: Users, href: "/appointments", label: "Appointments", category: "management" },
  { icon: Activity, href: "/health-campaigns", label: "Health Campaign Tracker", category: "management" },
  { icon: MessageCircle, href: "/feedback-referrals", label: "Feedback & Referrals", category: "management" },
...
export default function DashboardLayout({ children }: DashboardLayoutProps) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-slate-50">
      <header
        className={cn(
          "bg-white/90 backdrop-blur-md border-b border-slate-200/50 px-4 sm:px-6 py-4 fixed top-0 z-40 shadow-sm transition-all duration-300",
      ...
      >
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3 sm:gap-6">
            ...
            <Link href="/" className="flex items-center gap-2 sm:gap-3">
              ...
              <div className="hidden sm:block">
                <h1 className="text-lg sm:text-xl font-bold text-slate-800">ASTRA HEALTH</h1>
          ...
          <div className="flex items-center gap-2 sm:gap-3">
              ...
              <DropdownMenuTrigger asChild>
                <button className="flex items-center gap-2 sm:gap-3 rounded-xl bg-white/80 ring-1 ring-slate-200/50 px-2 sm:px-4 h-10 sm:h-12 hover:bg-white hover:ring-slate-300/50 transition-all duration-200 shadow-sm">
                  ...
                  <div className="text-left hidden sm:block">
                    <div className="text-sm font-semibold text-slate-800">Dr. Emma Hayes</div>
                    <div className="text-xs text-slate-500 -mt-0.5">General Practitioner</div>
                  </div>
                  <svg width="16" height="16" viewBox="0 0 24 24" className="text-slate-400 hidden sm:block">
            ...
            <Button
              variant="outline"
              className="h-10 sm:h-12 px-3 sm:px-4 bg-white/80 backdrop-blur-md border-slate-200/50 rounded-xl hover:bg-white hover:border-slate-300/50 transition-all duration-200 shadow-sm hidden xl:flex"
            >
              <Filter className="w-4 h-4 mr-2 text-slate-600" />
              <span className="text-slate-700">Customize</span>
            </Button>
      ...
      >
        <div className="p-4 border-b border-slate-200/30">
          ...
          >
            {sidebarExpanded || mobileSidebarOpen ? <X className="w-5 h-5" /> : <Menu className="w-5 h-5" />}
          </Button>
        </div>
        ...
        <nav className="flex-1 px-2 pb-4 overflow-y-auto">
                  ...
                  <Link key={item.href} href={item.href} onClick={() => setMobileSidebarOpen(false)}>
                    <Button
                      ...
                      className={cn(
                        ...
                        sidebarExpanded || mobileSidebarOpen ? "px-3 py-2 h-10" : "px-0 py-2 h-10 justify-center",
                        isActive
                    ...
                    >
                      <Icon className={cn("w-5 h-5", sidebarExpanded || mobileSidebarOpen ? "mr-3" : "")} />
                      {(sidebarExpanded || mobileSidebarOpen) && (
          ...
          <div className="space-y-1 mt-6">
            {(sidebarExpanded || mobileSidebarOpen) && (
              <div className="px-3 py-2">
                <h3 className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Health Services</h3>
              </div>
            )}
            {navigationItems
